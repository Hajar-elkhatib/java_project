                ...
                <c:if test="${not empty movie}">
                    ...
                    <div class="relative h-[60vh] md:h-[70vh] w-full overflow-hidden">
                        <div class="absolute inset-0">
                            ...
                            <div
                                class="absolute inset-0 bg-gradient-to-t from-netflix-black via-transparent to-netflix-black/60">
                            </div>
                        </div>
                        ...
                        <div
                            ...
                            <!-- Poster -->
                            <div
                                class="w-48 md:w-80 flex-none shadow-2xl rounded-lg overflow-hidden -mt-10 md:mt-0 transform -rotate-2 hover:rotate-0 transition-transform duration-500">
                            ...
                            <div class="flex-1 text-center md:text-left">
                                <div
                                    class="flex flex-wrap items-center justify-center md:justify-start gap-3 mb-4 text-xs md:text-sm font-semibold text-gray-400">
                                    <span class="text-green-400">★ ${movie.noteMoyenne} (${movie.nbVotes} votes)</span>
                                    <span>
                                        <fmt:formatDate value="${movie.dateSortie}" pattern="yyyy" />
                                    </span>
                                    <span
                                        class="px-2 border border-gray-600 rounded text-[10px]">${movie.typeContenu}</span>
                                ...
                                <p
                                    class="text-gray-300 text-base md:text-lg mb-8 leading-relaxed max-w-2xl mx-auto md:mx-0">
                                    ${movie.description}
                                </p>
                                ...
                                <div class="flex flex-wrap items-center justify-center md:justify-start gap-4">
                                    <c:if test="${not empty movie.trailerUrl}">
                                        <a href="#trailer"
                                            class="bg-white text-black py-3 px-8 rounded-md font-bold flex items-center gap-2 hover:bg-gray-200 transition-all shadow-lg active:scale-95">
                                    ...
                                    <c:if test="${not empty sessionScope.user}">
                                        <form
                                            action="${pageContext.request.contextPath}/favorites/${isFavorite ? 'remove' : 'add'}/${movie.id}"
                                            method="post">
                                            <button type="submit"
                                                class="bg-gray-500/30 text-white py-3 px-6 rounded-md font-bold flex items-center gap-2 hover:bg-white/20 transition-all border border-white/10">
                                    ...
                                    <c:if test="${not empty sessionScope.user}">
                                        <form action="${pageContext.request.contextPath}/movies/${movie.id}/rate"
                                            method="post"
                                            class="flex items-center bg-white/10 p-1 rounded-lg border border-white/10">
                                            <select name="note"
                                                class="bg-transparent text-white border-none text-sm focus:ring-0">
                                            ...
                                            <button type="submit"
                                                class="bg-netflix-red text-white py-1.5 px-3 rounded uppercase text-[10px] font-bold">Noter</button>
                                        </form>
                                    </c:if>
                                </div>
                            </div>
                    ...
                    <div class="max-w-7xl mx-auto px-4 md:px-12 py-16 grid lg:grid-cols-3 gap-12">
                        <div class="lg:col-span-2 space-y-16">
                            ...
                            <c:if test="${not empty movie.trailerUrl}">
                                <div id="trailer" class="scroll-mt-32">
                                    ...
                                    <div
                                        class="aspect-video w-full rounded-2xl overflow-hidden shadow-2xl border border-white/10 bg-black">
                                        <iframe src="${movie.getYouTubeEmbedUrl()}" class="w-full h-full"
                                            frameborder="0"
                        ...
                        <div class="space-y-8">
                            <div class="bg-netflix-darkGray p-8 rounded-2xl border border-white/5 shadow-2xl">
                                <h3 class="font-bold mb-6 uppercase tracking-widest text-xs text-gray-400">Score
                                    CineStream</h3>
                            ...
                            <c:if test="${not empty actors}">
                                <div class="bg-netflix-darkGray/50 p-8 rounded-2xl border border-white/5">
                                    <h3 class="font-bold mb-6 uppercase tracking-widest text-xs text-gray-400">Acteurs &
                                        Équipe</h3>
                                    <div class="space-y-4">
                                        <c:forEach items="${actors}" var="actor">
                                            <div class="flex items-center gap-3 group cursor-pointer">
                                                <div
                                                    class="w-10 h-10 bg-gray-700 rounded-full flex items-center justify-center font-bold text-xs uppercase group-hover:bg-netflix-red transition-colors">
                                                    ${actor.nom.substring(0,1)}
                                                </div>
                                                <div class="flex-1">
                                                    <p
                                                        class="text-sm font-bold group-hover:text-netflix-red transition-colors">
                                                        ${actor.nom}</p>
                                                    <p class="text-[10px] text-gray-500 uppercase">${actor.role} <c:if
                                                            test="${not empty actor.personnage}"> • ${actor.personnage}
                                                        </c:if>
                                                    </p>
                                                </div>
                                            </div>
